...
def try_stream(graph_app, inputs: Dict[str, Any]) -> Iterator[Tuple[str, Any]]:
    """
    Stream graph progress using 'values' mode (yields the full state at each step).
    This avoids running the graph twice.
    """
    last_state = inputs
    try:
        ...
        # Fallback to invoke only if streaming fails
    ...
    for kind, payload in try_stream(app, inputs):
        if kind == "values":
            ...
            summary = {
                ...
                "queries": len(current_state.get("queries", []) or []),
                "evidence": len(current_state.get("evidence", []) or []),
                "tasks": len((current_state.get("plan") or {}).get("tasks", [])) if current_state.get("plan") else 0,
                "sections": len(current_state.get("sections", []) or []),
                "images": len(current_state.get("image_specs", []) or []),
            }
            progress_area.json(summary)
            log(f"[{kind}] Step completed.")
