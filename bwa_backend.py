...
IMAGE_MODEL = os.environ.get("POLLINATIONS_MODEL", "flux")   # flux | turbo | ...
...
def _pollinations_generate_image_bytes(prompt: str) -> bytes:
    ...
    """
    encoded = _url_quote(prompt)
    url = f"https://image.pollinations.ai/prompt/{encoded}"
    params = {"width": 1280, "height": 720, "model": IMAGE_MODEL, "nologo": "true"}
...
def _hf_generate_image_bytes(prompt: str) -> bytes:
    ...
    Uses HUGGINGFACE_API_KEY from .env (model: black-forest-labs/FLUX.1-schnell).
    """
...
def _generate_image_bytes(prompt: str) -> bytes:
    """Try Pollinations first; fall back to HuggingFace."""
    try:
        return _pollinations_generate_image_bytes(prompt)
    except Exception as primary_err:
        print(f"[image] Pollinations failed ({primary_err}). Trying HuggingFace…")
        return _hf_generate_image_bytes(prompt)


def _compact_image_error(err: Exception) -> str:
    text = str(err)
    upper = text.upper()
    if "RESOURCE_EXHAUSTED" in upper or "QUOTA" in upper or "429" in upper:
        return (
            "Image generation quota exhausted for this API project/key. "
            "Check AI Studio Rate Limits/Billing, or switch key/project/model."
        )
    return text[:300]
        ...
        if not out_path.exists():
            try:
                ...
                prompt_block = (
                    ...
                    f"> **Model:** {IMAGE_MODEL}\n>\n"
                    f"> **Error:** {_compact_image_error(e)}\n"
                )
